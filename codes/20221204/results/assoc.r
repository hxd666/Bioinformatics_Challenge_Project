...
if (subject == 'all'){
  ...
  for (.pheno in pheno_list_categorical){
    ...
    # Fit Two:
    football_subset <- !is.na(pheno$footyrs)
...
if (subject == 'cases'){
  ...
  for (.pheno in pheno_list_categorical){
    ...
    # Fit Two:
    footbcases_subset <- !is.na(pheno$footyrs)
...
if (subject == 'age'){
  ...
  for (.pheno in pheno_list_categorical){
    ...
    # Fit One All:
    subset_older <- !is.na(pheno[, predictor]) & pheno$agedeath > median_age
    subset_younger <- !is.na(pheno[, predictor]) & pheno$agedeath <= median_age
    # ---- older:
    ...
    model.age.data <- rbind(model.age.data, output.row)
    # Fit One Cases:
    subset_older <- !is.na(pheno[, predictor]) & pheno$agedeath > median_age & pheno$CTE == '1'
    subset_younger <- !is.na(pheno[, predictor]) & pheno$agedeath <= median_age & pheno$CTE == '1'
    # ---- older:
    fit <- with(imp, polr(as.formula(model1.form), na.action="na.omit", Hess = T, subset = subset_older))
    pool.fit <- pool(fit)
    result <- summary(pool.fit)
    ...
    # Fit Two All:
    subset_older <- !is.na(pheno[, predictor]) & pheno$agedeath > median_age & !is.na(pheno$footyrs)
    subset_younger <- !is.na(pheno[, predictor]) & pheno$agedeath <= median_age & !is.na(pheno$footyrs)
    # ---- older:
    ...
    model.age.data <- rbind(model.age.data, output.row)
    # Fit Two Cases:
    subset_older <- !is.na(pheno[, predictor]) & pheno$agedeath > median_age & pheno$CTE == '1' & !is.na(pheno$footyrs)
    subset_younger <- !is.na(pheno[, predictor]) & pheno$agedeath <= median_age & pheno$CTE == '1' & !is.na(pheno$footyrs)
    # ---- older:
    fit <- with(imp, polr(as.formula(model2.form), na.action="na.omit", Hess = T, subset = subset_older))
    pool.fit <- pool(fit)
    result <- summary(pool.fit)
