...
# Load Data
load('/restricted/projectnb/cte/Challenge_Project_2022/xdhan/scripts/20230222_3/imputation.RData')
# Set PCs formula
...
# check common H1B1G1
subhap <- read.csv('/restricted/projectnb/cte/Challenge_Project_2022/imputation/neural_net/Prediction_data_updatedSD_PredResult.txt',
                   as.is = T)
subhap$H1B1G1 <- ifelse(subhap$Class == 2, 0, 1)
...
colnames(subhap)[3] <- 'H1B1G1.nn'

batch.subhap <- rbind(batch1.subhap, batch2.subhap)
batch.subhap$TOPMed_ID[grepl('k-', batch.subhap$TOPMed_ID)] <- sapply(batch.subhap$TOPMed_ID[grepl('k-', batch.subhap$TOPMed_ID)], function(x){gsub('k-', 'K-',x)})
batch.subhap <- batch.subhap[batch.subhap$TOPMed_ID %in% c(subhap.batch1$ID, subhap.batch2$ID), ]
batch.subhap$H1B1G1 <- ifelse(batch.subhap$H1B1G1 == 2, 1, batch.subhap$H1B1G1)

batch.subhap <- merge(batch.subhap, subhap, by.x = 'TOPMed_ID', by.y = 'ID')

common.ID <- batch.subhap$TOPMed_ID[batch.subhap$H1B1G1 == batch.subhap$H1B1G1.nn]
common.ID <- na.omit(common.ID)
...
for (predictor in pred.list){
  ...
  for (.pheno in c(pheno_list_binary, pheno_list_categorical, pheno_list_continuous)){
    ...
    freq <- round(sum(pheno[,predictor][assoc_subject])/n, 3)
