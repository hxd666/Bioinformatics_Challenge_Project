...
# Load Data
load('/restricted/projectnb/cte/Challenge_Project_2022/xdhan/scripts/20230323/imputatoin.RData')
# Set PCs formula
pred.list   <- read.table('hla.filtered.info.txt', as.is = T, header = T)
pred.list   <- pred.list$ID
pred.list   <- sapply(pred.list, function(x){gsub('\\*','.',x)})
pred.list   <- sapply(pred.list, function(x){gsub(':','.',x)})
...
for (predictor in pred.list){
  ...
  for (.pheno in "CA2"){
    ...
    tryCatch({
      ...
      output.row <<- c('All', '1', .pheno, predictor, NA, NA, NA, NA, NA)
    })
...
model.all.data <- na.omit(model.all.data)
...
#                                                                              method = 'fdr')
# save result
outputname <- 'results/Model1.Euro.HLA.CA2.assoc.txt'
