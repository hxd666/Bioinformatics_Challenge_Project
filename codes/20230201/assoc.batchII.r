...
for (predictor in pred.list){
  ...
  for (.pheno in c(pheno_list_binary, pheno_list_categorical, pheno_list_continuous)){
    ...
    # Fit One:
    subset <- !is.na(pheno[, predictor]) & euro_subset & pheno$Batch == 'II'
...
model.all.data$A1 <- 'A'
model.all.data$A2 <- 'T'
model.all.data$SNP <- paste(model.all.data$Phenotype, model.all.data$Predictor, sep = "_")
model.all.data <- model.all.data[model.all.data$Freq >= 0.05,]
...
model.all.data$FDR[model.all.data$Phenotype %in% endophenotypes &
                     model.all.data$Predictor %in% dosages] <- p.adjust(model.all.data$P.value[model.all.data$Phenotype %in% endophenotypes &
                                                                              ...
                                                                              method = 'fdr')
# save result
outputname <- 'results/Model1.Euro.assoc.batch2.txt'
