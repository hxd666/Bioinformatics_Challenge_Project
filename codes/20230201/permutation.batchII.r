...
# Permutation Test
L <- 1000
N <- nrow(complete(imp, 1)[complete(imp,1)$race %in% "1" & complete(imp,1)$Batch %in% 'II', ])
...
for (l in (1:L)) {
  ...
  assoc.list <- list()
  for (iter in (1:10)){
    l.imp.iter <- complete(imp, iter)[complete(imp,1)$race %in% "1" & complete(imp,1)$Batch %in% 'II', ]
  ...
  for (.pheno in c(pheno_list_binary, pheno_list_categorical, pheno_list_continuous)){
    ...
    subset <- !is.na(pheno[, predictor]) & euro_subset & pheno$Batch %in% 'II'
...
output.filename <- 'permutations/XXXX.permutation.batch2.P.values'
