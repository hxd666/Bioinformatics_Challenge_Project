...
#------------------------------------------------------
dir.out <-		"/restricted/projectnb/cte/Challenge_Project_2022/xdhan/scripts/GWAS_UNITE/endophenotypes/"
...
comp.func.glm_dx.int<-  function(df,i.freq,i.marker,i.var,i.data.dos1,i.num.samp,outFile)  {
  ...
  if (i.freq<=0.01 | i.freq>=0.99 | is.na(i.freq) | i.var < 0.4)  {
    ...
    tryCatch({
      ...
      lme1 <- lapply(i.data.dos1, function(x){polr(i.form, x, Hess = T, na.action = na.omit)})
