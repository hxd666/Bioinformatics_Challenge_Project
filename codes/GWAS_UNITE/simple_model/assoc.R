...
risk   <- t.args[1] # CTE or CTE stage
...
#------------------------------------------------------
dir.out <-		"/restricted/projectnb/cte/Challenge_Project_2022/xdhan/scripts/GWAS_UNITE/simple_model/"
...
#------------------------------------------------------
fn.pheno <-	"/restricted/projectnb/cte/Challenge_Project_2022/data/pheno/update_11182022/APOEAnalysis_UNITE_wo_unmatched.csv"
#--- set up the pheotype file
cat( fn.pheno, fill=T )
if( !file.exists(fn.pheno) ){
  cat("[ERROR] No phenotype file"); quit()
}

data.pheno <- read.csv(fn.pheno, as.is = T)
# filter by age
data.pheno <- data.pheno[data.pheno$agedeath >= 20, ]
# filter by race
data.pheno <- data.pheno[data.pheno$race %in% c(1, 'White'), ]
#------------------------------------------------------
#--- set up the covariates 
#------------------------------------------------------
age.col.name<-	"agedeath"
ph.col.name<-			risk
data.pheno <- data.pheno[, c("subjid", "pin", age.col.name, ph.col.name)]

if (risk == 'CTE'){
  data.pheno$CTE[data.pheno$CTE == 'Yes'] <- 1
  data.pheno <- data.pheno[complete.cases(data.pheno[,c("agedeath", ph.col.name)]), ]
  data.pheno$CTE <- as.numeric(data.pheno$CTE)
} else if (risk == 'CTEstage'){
  data.pheno <- data.pheno[complete.cases(data.pheno[,c("agedeath", ph.col.name)]), ]
  data.pheno$CTEstage[data.pheno$CTEstage == 'Stage IV'] <- '4'
  data.pheno$CTEstage <- as.factor(data.pheno$CTEstage)
}

# PCs file
if (study == 'batch1'){
  data.pcs <- read.table('/restricted/projectnb/cte/Challenge_Project_2022/xdhan/_PCA/UNITE_DIAGNOSE/UNITE_batch1/PCA/UNITE_batch1.pca.txt',
                         header = F, as.is = T)
  colnames(data.pcs) <- c('pin', sapply(1:20, function(x){paste0('PC',x)}))
  pcs.col.name<-	unname(sapply(1:10, function(x){paste0('PC',x)}))
  data.pheno <- merge(data.pheno, data.pcs, by = 'pin')
} else if (study == 'batch2'){
  data.pcs <- read.table('/restricted/projectnb/cte/Challenge_Project_2022/xdhan/_PCA/UNITE_DIAGNOSE/UNITE_batch2/PCA/UNITE_batch2.pca.txt',
                         header = F, as.is = T)
  colnames(data.pcs) <- c('subjid', sapply(1:20, function(x){paste0('PC',x)}))
  pcs.col.name<-	unname(sapply(1:10, function(x){paste0('PC',x)}))
  data.pheno <- merge(data.pheno, data.pcs, by = 'subjid')
}
...
if (study == 'batch1'){
  ...
  data.sample <- data.sample$V1[match(data.pheno$pin, data.sample$V1)]
  ...
  sample.dir <- gsub("XXXX",chr,sample.dir)
  data.sample <- read.table(sample.dir, header = F, as.is = T)
  data.sample <- data.sample$V1[match(data.pheno$subjid, data.sample$V1)]
...
comp.func.glm_dx.int<-  function(df,i.freq,i.marker,i.var,i.data.dos1,i.num.samp,outFile)  {
  ...
  if (i.freq<=0.01 | i.freq>=0.99 | is.na(i.freq) | i.var < 0.4)  {
    ...
    tryCatch({
      ...
      if (risk == 'CTE'){
        lme1 <- glm(i.form, data = i.data.dos1, na.action = na.omit, family = binomial())
        i.glm.out.table<- summary(lme1)$coefficients
      } else if (risk == 'CTEstage'){
        lme1 <- polr(i.form, data = i.data.dos1, na.action = na.omit, Hess = T)
        i.glm.out.table <- coeftest(lme1)
      }
      ...
      #---	snp term
      if ( sum( rownames( i.glm.out.table ) %in% i.marker ) == 1 )    {
        i.glm.out.snp<-	as.vector( i.glm.out.table[rownames( i.glm.out.table ) %in% i.marker,])
      }	else	{
...
#cmd.tabix <- 'less /restricted/projectnb/cte/Challenge_Project_2022/xdhan/scripts/GWAS_UNITE/simple_model/pipe.txt'
...
while( length( stream<- readLines( pipe.tabix, n=buffer.nlines) ) > 0 )    {
  ...
  if( nrow( data.pheno ) != nrow( i.data.tdos1 ) )       {
  ...
  if (study == 'batch1'){
    i.data.tdos1$ID<-			data.pheno$pin
  } else if (study == 'batch2'){
    i.data.tdos1$ID<-     data.pheno$subjid
  }
  
  #---	merge the dosage data and the phenotype/covariate data
  i.data.tdos1.0<-	i.data.tdos1
  i.data.tdos1<-	cbind(data.pheno, i.data.tdos1)	
  ...
  for (.i in 1:nrow(i.data.info)){
    ...
    i.bp<-		df['BP']
    i.chr <- gsub('chr','',df['CHR'])
    i.data.dos1<-	i.data.tdos1[, c('ID',ph.col.name, all.covar.names, i.marker) ]
    i.data.dos1<-	na.omit( i.data.dos1 )
    i.num.samp<-	nrow( i.data.dos1 )
    
    i.freq<-		mean( i.data.dos1[, c(i.marker)], na.rm=T ) / 2
    
    if (i.freq==1 | i.freq==0 | is.na(i.freq))	{
      i.var<-	NA
    }	else	{
      i.var<-	var ( i.data.dos1[, c(i.marker)], na.rm=T ) / (2*i.freq*(1-i.freq))
    }
