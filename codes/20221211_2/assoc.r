...
# three phenotypes: CTE, CTEstage, Dementia
three.pheno   <- c(pheno_list_binary, 'CTEstage')
subhaplotypes <- pred.list[1:9]
dosages       <- pred.list[10:14]
model.all.data$FDR[model.all.data$Phenotype %in% three.pheno & 
                     model.all.data$Predictor %in% subhaplotypes] <- p.adjust(model.all.data$P.value[model.all.data$Phenotype %in% three.pheno & 
                                                                                                       model.all.data$Predictor %in% subhaplotypes],
                                                                              method = 'fdr')

model.all.data$FDR[model.all.data$Phenotype %in% three.pheno & 
                     model.all.data$Predictor %in% dosages] <- p.adjust(model.all.data$P.value[model.all.data$Phenotype %in% three.pheno & 
