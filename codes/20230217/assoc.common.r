...
# Load Data
load('/restricted/projectnb/cte/Challenge_Project_2022/xdhan/scripts/20230217/imputation.RData')
# Set PCs formula
...
pheno.impute <- read.table('/restricted/projectnb/cte/Challenge_Project_2022/xdhan/scripts/20230201/pheno.txt',
                           header = T, as.is = T)
pheno.impute <- pheno.impute[match(pheno$subjid, pheno.impute$subjid), ]
euro_subset <- euro_subset & (pheno$H1B1G1 == pheno.impute$H1B1G1)
euro_subset[is.na(euro_subset)] <- F
# ==============================================================================================
...
model.all.data$FDR[model.all.data$Phenotype %in% endophenotypes & 
                     model.all.data$Predictor %in% dosages] <- p.adjust(model.all.data$P.value[model.all.data$Phenotype %in% endophenotypes & 
                                                                              ...
                                                                              method = 'fdr')
# save result
outputname <- 'Model1.Euro.common.assoc.txt'
