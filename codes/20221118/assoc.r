...
# lm.pcs <- polr(as.formula(paste('CTEstage', pcs.formula, sep = ' ~ ')), data = pheno, 
#                na.action = na.omit, Hess = T)
# summary(lm.pcs)$coefficients


euro.batch1 <- pheno$race %in% 1 & pheno$Batch == 'I'
euro.batch2 <- pheno$race %in% 1 & pheno$Batch == 'II'
...
model1.data <- data.frame('Batch' = 0,'Phenotype' = 0, 'Predictor'= 0, 'N' = 0,
...
euro.batch1.cases <- pheno$race %in% 1 & pheno$Batch == 'I' & pheno$CTE == 1
euro.batch2.cases <- pheno$race %in% 1 & pheno$Batch == 'II' & pheno$CTE == 1
...
model2.data <- data.frame('Batch' = 0,'Phenotype' = 0, 'Predictor'= 0, 'N' = 0,
                          'Effect' = 0, 'SE' = 0, 'P-value' = 0, stringsAsFactors = F)
for (.pheno in pheno_list_categorical){
  print(.pheno)
...
euro.batch1.football <- pheno$race %in% 1 & pheno$Batch == 'I' & !is.na(pheno$footyrs)
euro.batch2.football <- pheno$race %in% 1 & pheno$Batch == 'II' & !is.na(pheno$footyrs)
...
model3.data <- data.frame('Batch' = 0,'Phenotype' = 0, 'Predictor'= 0, 'N' = 0,
                          'Effect' = 0, 'SE' = 0, 'P-value' = 0, stringsAsFactors = F)
for (.pheno in pheno_list_categorical){
  print(.pheno)
...
model4.data <- data.frame('Batch' = 0,'Phenotype' = 0, 'Predictor'= 0, 'N' = 0,
                          'Effect' = 0, 'SE' = 0, 'P-value' = 0, stringsAsFactors = F)
for (.pheno in pheno_list_categorical){
  print(.pheno)
...
# model 5 Phenotype ~ H1_dosage + Age + PCs for subjects with age > median
median_age_batch1 <- median(pheno$agedeath[pheno$Batch=='I'])
median_age_batch2 <- median(pheno$agedeath[pheno$Batch=='II'])
euro.batch1.old <- pheno$race %in% 1 & pheno$Batch == 'I' & pheno$agedeath >= median_age_batch1
euro.batch2.old <- pheno$race %in% 1 & pheno$Batch == 'II' & pheno$agedeath >= median_age_batch2
model5 <- function(pheno1, batch){
  form <- paste(pheno1, " ~ H1_dosage + agedeath + ", pcs.formula, sep = '')
  fit <- with(imp, polr(as.formula(form), na.action="na.omit", Hess = T, subset = batch))
  pool.fit <- pool(fit)
  result <- summary(pool.fit)
  if (all(batch == euro.batch1.old)){
    batch.n = 'I'
  } else {
    batch.n = 'II'
  }
...
model5.data <- data.frame('Batch' = 0,'Phenotype' = 0, 'Predictor'= 0, 'N' = 0,
                          'Effect' = 0, 'SE' = 0, 'P-value' = 0, stringsAsFactors = F)
for (.pheno in pheno_list_categorical){
  print(.pheno)
  model5.data <- rbind(model5.data, model5(.pheno, euro.batch1.old))
  model5.data <- rbind(model5.data, model5(.pheno, euro.batch2.old))
}
