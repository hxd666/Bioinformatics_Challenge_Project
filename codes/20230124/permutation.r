...
model.all.simulation <- matrix(0, nrow = 1, ncol = (2+length(c(pheno_list_binary, pheno_list_categorical, pheno_list_continuous))))
model.all.simulation <- as.data.frame(model.all.simulation)
colnames(model.all.simulation) <- c('Predictor', 'Simulation', pheno_list_binary, pheno_list_categorical, pheno_list_continuous)
...
for (l in (1:L)) {
  ...
  for (.pheno in c(pheno_list_binary, pheno_list_categorical, pheno_list_continuous)){
    ...
    if (.pheno %in% c("CTE", "DementiaHx")){
      ...
      fit <- lapply(assoc.list, function(x){glm(as.formula(model1.form), x, na.action="na.omit", family=gaussian(link="identity"), subset = subset)})    }
    pool.fit <- pool(fit)
...
output.filename <- 'permutations/XXXX.permutation.P.values'
